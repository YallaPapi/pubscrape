...
export class DatabaseManager {
  ...
  async loadFromCache(names: string[], settings: Settings): Promise<Record<string, DbType | undefined>> {
    ...
    while (databasesToLoad.length > 0) {
      ...
      const cachedDBsAvailable = await settings.getFromStorage('cachedDatabases') as boolean;
      console.debug(`DBM: Cached databases available: ${cachedDBsAvailable}`);

      if (!loadedFromCache && cachedDBsAvailable === true && !!indexedDB) {
      ...
      const idbStorageDatabasesAvailable = await settings.getFromStorage('idbStorageDatabases') as boolean;
      console.debug(`DBM: IdbStorage databases available: ${idbStorageDatabasesAvailable}`);

      if (!loadedFromIdbStorage && idbStorageDatabasesAvailable === true) {
