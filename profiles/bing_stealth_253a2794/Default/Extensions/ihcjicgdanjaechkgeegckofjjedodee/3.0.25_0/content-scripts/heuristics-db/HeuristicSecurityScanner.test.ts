...
jest.mock('@/app/scripts/fpcheck/fp-checker', () => ({
    FPChecker: jest.fn().mockImplementation(() => ({
        shouldBlock: jest.fn().mockResolvedValue(true) // Default to true (block) so tests continue execution
    }))
}));
...
describe('HeuristicSecurityScanner', () => {
    ...
    beforeEach(() => {
        ...
        // Reset FPChecker mock
        const { FPChecker } = require('@/app/scripts/fpcheck/fp-checker');
        mockFPChecker = {
            shouldBlock: jest.fn().mockResolvedValue(true) // Default to true (block)
        };
        FPChecker.mockImplementation(() => mockFPChecker);
    ...
    describe('scanForThreats', () => {
        ...
        it('should skip onScam() when FPChecker returns good result', async () => {
            // Mock FPChecker to return false (don't block) for this test
            mockFPChecker.shouldBlock.mockResolvedValue(false);

            mockMalwarebytes.isWhitelisted.mockReturnValue(false);
            
            await scanner.scanForThreats(mockUrls, 'http://scam.com', 1);

            // Should not call onScam when FPChecker returns false (don't block)
            expect(onScam).not.toHaveBeenCalled();
            expect(chrome.tabs.update).not.toHaveBeenCalled();
        });
    });
});